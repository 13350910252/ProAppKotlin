 ...
 * 这样写也有缺点就是很多方法是public的
 **/

open class SkipActivity : FragmentActivity() {
    /**
     * start activity for result deque
     */
    @PublishedApi
    internal lateinit var startActivityResultDeque: LinkedBlockingDeque<StartActivityResult>
     ...
     * {@inheritDoc}
     */
    private   fun initManageStartActivity() {
    ...
    inline fun <reified T> gotoActivityForResult(
        ...
        gotoActivityForResult<T>({}, options, result)
    }

    inline fun <reified T> gotoActivityForResult(noinline result: StartActivityResult) {
        gotoActivityForResult<T>({ null }, result)
    }
