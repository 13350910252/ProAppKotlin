...
object PathParser {
    ...
    fun createPathFromPathData(pathData: String): Path? {
        var path: Path? = Path()
        try {
            createNodesFromPathData(pathData)?.let {
    ...
    fun deepCopyNodes(source: Array<PathDataNode?>?): Array<PathDataNode?>? {
        return source?.let {
            val copy: Array<PathDataNode?> = arrayOfNulls(it.size)
            for (i in it.indices) {
                it[i]?.also { it2 ->
                    copy[i] = PathDataNode(it2)
                }
            }
            copy
        } ?: source
    ...
    class PathDataNode {
        ...
        fun interpolatePathDataNode(
            ...
            for (i in nodeFrom.params.indices) {
                params[i] =
                    nodeFrom.params.get(i) * (1 - fraction) + nodeTo.params.get(i) * fraction
            }
        }
        ...
        companion object {
            ...
            fun nodesToPath(
                ...
                for (i in node.indices) {
                    PathDataNode.Companion.addCommand(
                        path!!,
                        current,
                        previousCommand,
                        node[i].type,
            ...
            private fun addCommand(
                ...
                previousCmd: Char, cmd: Char, vall: FloatArray
            ) {
                ...
                while (k < vall.size) {
                    when (cmd) {
                        'm' -> {
                            currentX += vall[k + 0]
                            currentY += vall[k + 1]
                            if (k > 0) { // According to the spec, if a moveto is followed by multiple
                                // pairs of coordinates, the subsequent pairs are treated as
                                // implicit lineto commands.
                                path.rLineTo(vall[k + 0], vall[k + 1])
                            } else {
                                path.rMoveTo(vall[k + 0], vall[k + 1])
                        ...
                        'M' -> {
                            ...
                            if (k > 0) { // According to the spec, if a moveto is followed by multiple
                                // pairs of coordinates, the subsequent pairs are treated as
                                // implicit lineto commands.
                                path.lineTo(vall[k + 0], vall[k + 1])
                            } else {
                                path.moveTo(vall[k + 0], vall[k + 1])
                                currentSegmentStartX = currentX
                                currentSegmentStartY = currentY
                            }
                        }
                        ...
                        'h' -> {
                            path.rLineTo(vall[k + 0], 0f)
                            currentX += vall[k + 0]
                        }
                        'H' -> {
                            path.lineTo(vall[k + 0], currentY)
                            currentX = vall[k + 0]
                        }
                        'v' -> {
                            path.rLineTo(0f, vall[k + 0])
                            currentY += vall[k + 0]
                        }
                        'V' -> {
                            path.lineTo(currentX, vall[k + 0])
                            currentY = vall[k + 0]
                        }
                        'c' -> {
                            path.rCubicTo(
                                vall[k + 0], vall[k + 1], vall[k + 2], vall[k + 3],
                                vall[k + 4], vall[k + 5]
                            )
                            ctrlPointX = currentX + vall[k + 2]
                            ctrlPointY = currentY + vall[k + 3]
                            currentX += vall[k + 4]
                            currentY += vall[k + 5]
                        }
                        'C' -> {
                        ...
                        's' -> {
                            ...
                            ctrlPointX = currentX + vall[k + 0]
                            ctrlPointY = currentY + vall[k + 1]
                            currentX += vall[k + 2]
                            currentY += vall[k + 3]
                        }
                        'S' -> {
                        ...
                        'q' -> {
                            path.rQuadTo(vall[k + 0], vall[k + 1], vall[k + 2], vall[k + 3])
                            ctrlPointX = currentX + vall[k + 0]
                            ctrlPointY = currentY + vall[k + 1]
                            currentX += vall[k + 2]
                            currentY += vall[k + 3]
                        ...
                        'Q' -> {
                            path.quadTo(vall[k + 0], vall[k + 1], vall[k + 2], vall[k + 3])
                            ctrlPointX = vall[k + 0]
                            ctrlPointY = vall[k + 1]
                            currentX = vall[k + 2]
                            currentY = vall[k + 3]
                        ...
                        'A' -> {
                            PathDataNode.Companion.drawArc(
                                path,
                                currentX,
                                currentY,
                                vall[k + 5],
            ...
            private fun drawArc(
                ...
                PathDataNode.Companion.arcToBezier(
                    p,
                    cx,
                    cy,
                    a.toDouble(),
            ...
            private fun arcToBezier(
                ...
                var e1y = e1y
                val numSegments =
                    Math.ceil(Math.abs(sweep * 4 / Math.PI)).toInt()
                var eta1 = start
